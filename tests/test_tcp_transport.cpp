...
static std::pair<int, int> make_socket_pair() {
    ...
    fiasco::detail::set_nonblocking(fds[0]);
    fiasco::detail::set_nonblocking(fds[1]);
    return {fds[0], fds[1]};
}
...
TEST_CASE("socket_fd takes ownership and closes on destruction",
    ...
    {
        fiasco::detail::socket_fd fd(raw);
        REQUIRE(fd.valid());
        REQUIRE(fd.get() == raw);
    }
...
TEST_CASE("socket_fd is movable", "[tcp_transport]") {
    ...
    fiasco::detail::socket_fd a(raw);
    fiasco::detail::socket_fd b(std::move(a));
...
TEST_CASE("socket_fd move-assignment transfers ownership", "[tcp_transport]") {
    ...
    fiasco::detail::socket_fd a(r1);
    fiasco::detail::socket_fd b(r2);
    b = std::move(a);
...
TEST_CASE("set_nonblocking sets O_NONBLOCK on a valid fd", "[tcp_transport]") {
    ...
    REQUIRE_NOTHROW(fiasco::detail::set_nonblocking(raw));
...
TEST_CASE("close_fd is a no-op for fd < 0", "[tcp_transport]") {
    ...
    REQUIRE_NOTHROW(fiasco::detail::close_fd(-1));
...
TEST_CASE("send_all transmits all bytes", "[tcp_transport]") {
    ...
    REQUIRE(fiasco::detail::send_all(writer, data));
...
TEST_CASE("send_all returns true for an empty string", "[tcp_transport]") {
    ...
    REQUIRE(fiasco::detail::send_all(writer, ""));
...
TEST_CASE("send_all transmits multi-kilobyte payloads correctly",
    ...
    while (total < data.size()) {
        ssize_t n =
            ::recv(reader, received.data() + total, data.size() - total, 0);
        if (n <= 0) {
            break;
        }
...
TEST_CASE("send_all returns false on a closed socket", "[tcp_transport]") {
    ...
    // May need more than one attempt on some kernels before SIGPIPE/EPIPE
    // fires. MSG_NOSIGNAL prevents the signal; EPIPE triggers the false return.
    bool result = fiasco::detail::send_all(writer, std::string(1024, 'a'));
